...
service.interceptors.response.use(
  (response: AxiosResponse<BaseResponse>) => {
    ...
    if (res.code !== ResultEnum.SUCCESS) {
      $message.open({ content: res.message || UNKNOWN_ERROR, type: 'error' })
      // Illegal token
  ...
  (error) => {
    if (!(error instanceof CanceledError)) {
      ...
      $message.open({ content: errMsg, key: errMsg, type: 'error' })
...
export async function request(_url: string | RequestOptions, _config: RequestOptions = {}) {
  ...
  try {
    ...
    if (hasSuccess) {
      ...
      if (successMsg) {
        $message.open({ content: successMsg, type: 'success' })
      } else if (showSuccessMsg && message) {
        $message.open({ content: message, type: 'success' })
      }
    }
